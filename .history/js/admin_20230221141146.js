...
function getAllAccounts() {
  ...
  var arr = [];
  $.ajax({
    url: "http://192.168.1.19:8386/users/getAllAccounts",
    type: "GET",
    headers: {
    ...
    success: (data) => {
      ...
      if (data.status == 0) {
        ...
        arrPagination.forEach((el, index) => {
...
function deleteAccount(id) {
  ...
  $.ajax({
    url: `http://192.168.1.19:8386/users/deleteAccount/${id}`,
    type: "DELETE",
    headers: {
      Authorization: `Bearer ${getCookie("token")}`,
    },
...
function searchByName() {
  ...
  for (i = 0; i < name.length; i++) {
    const blockKey = document.getElementsByClassName("blockKey");\    if (
...
function updateAccount(id) {
  ...
  $.ajax({
    url: `http://192.168.1.19:8386/users/updateAccount`,
    type: "POST",
    headers: {
      Authorization: `Bearer ${getCookie("token")}`,
    },
...
function CreateAccount() {
  ...
  $.ajax({
    url: `http://192.168.1.19:8386/users/signUp`,
    headers: {
      Authorization: `Bearer ${getCookie("token")}`,
    },
    method: "POST",
